...
import { SnackToastService, DeleteConfirmComponent, SnackbarModel, AlertType, AlertTypeClass } from '../shared';
...
export class ReportComponent implements AfterViewInit, OnDestroy {
    ...
    displayedColumns = ['select', 'id', 'action'];
    ...
    onAction(id: number) {
        let instance: MatDialogRef<ReportFormComponent, ResponseModel>;
        const data = this.dataSource.data.find(_ => _.id === id);
        instance = this.dialog.open(ReportFormComponent, {
            width: '900px',
            data: data ? data : {},
            autoFocus: false,
        });
    ...
    private initData() {
        ...
        this.rService.getReports(this.query).pipe(takeUntil(this.toDestroy$), delay(600)).subscribe(res => {
            this.cdr.markForCheck();
    ...
    onDelete(id: number) {
        ...
        instance.afterClosed()
            .pipe(
                takeUntil(this.toDestroy$),
                filter(yes => yes),
                switchMap(() => this.rService.deleteReport(id)
