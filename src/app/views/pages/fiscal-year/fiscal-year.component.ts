...
import { fadeIn, fadeInOutStagger, collectionInOut, ParamGenService, ExtendedMatDialog, CustomAnimationPlayer } from '../../../../../src/app/utils';
...
export class FiscalYearComponent implements AfterViewInit, OnDestroy {
    ...
    onAction(id: number) {
        let instance: MatDialogRef<FYFormComponent, ResponseModel>;
        const data = this.dataSource.data.find(_ => _.id === id);
        instance = this.dialog.open(FYFormComponent, {
            width: '650px',
            data: data ? data : {},
            autoFocus: false,
        });
    ...
    private initData() {
        ...
        this.fyService.getList().pipe(takeUntil(this.toDestroy$), delay(1000)).subscribe(res => {
            this.cdr.markForCheck();
    ...
    onDelete(id: number) {
        ...
        instance.afterClosed()
            .pipe(
                takeUntil(this.toDestroy$),
                filter(yes => yes),
                switchMap(() => this.fyService.delete(id)
